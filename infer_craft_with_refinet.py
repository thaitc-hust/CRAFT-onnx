...
parser.add_argument('--refineonnxpath', type=str, default='onnx_model/refine.onnx', help='path refine onnx model')
...
refisess = rt.InferenceSession(opt.refineonnxpath)
refinput_name_y = refisess.get_inputs()[0].name
refinput_name_feature = refisess.get_inputs()[1].name
# print(refisess.get_inputs()[1])
...
y_refiner = torch.tensor(refisess.run(None, {refinput_name_y: y, refinput_name_feature: feature})[0])
...
if len(bboxes_xxyy) >0:
    ...
    cv2.imwrite('result/result_with_refinet.jpg', img)